...
void send_package(char *package, int size){
	sendto(nw_state.sockfd, (const char *) package, size, 0, (const struct sockaddr *) &receiver_addr, sizeof(receiver_addr));
}
...
unsigned int receive_package(){
	unsigned int sender_addr_len;
	int msg_len = recvfrom(nw_state.sockfd, buffer, BUFSIZE, 0, (struct sockaddr *) &sender_addr, &sender_addr_len);
	return (msg_len > 0) ? msg_len : 0;
}
